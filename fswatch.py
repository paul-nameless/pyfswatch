...
class Monitor:
    ...
    def __init__(self):
        ...
        def _callback_wrapper(events, event_num):
            event = events[0]
            self._callback(event.path, event.evt_time,
                           event.flags, event.flags_num, event_num)
    ...
    def set_callback(self, callback):
        ...
        assert libfswatch.fsw_set_callback(
            self.handle, self.cevent_callback) == 0
